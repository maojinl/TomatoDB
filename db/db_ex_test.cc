...
class DBTest : public testing::Test {
 public:
  ...
  Status Put(const std::string& k, const std::string& v) {
    return dbfull()->Put(*params, k, v);
  }

  Status Delete(const std::string& k) { 
    return dbfull()->Delete(*params, k); 
  }
...
static void MTThreadBody(void* arg) {
  ...
  WriteParams* params = new WriteParams(pDb);
  ...
  do {
    ...
    for (int step = 0; step < N; step++) {
      ...
      if (p < 45) {  // Put
        ...
        WriteOptions o;
        auto pDb = dbfull();
        auto params1 = new WriteParams(pDb);
