...
char* MemoryHandler::AllocateAligned(size_t size) {
  return Allocate(size, true);
}

char* MemoryHandler::Allocate(size_t size, bool aligned) {
  ...
  do {
    ...
    if (Remain == 0) {
      ...
      return this->skip_header((char*)p);
    }
    if (Remain > 0) {
      if (Remain < Min) {
        Min = Remain;
...
int MemoryHandler::Destroy(void* iFree) {
  ...
  if (iFree == nullptr || iFree <= mpHead || iFree >= mpHead + mAllocBaseSize) {
    ret = 0xfffffffe;
    return ret;
  }
